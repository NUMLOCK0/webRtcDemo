...
pc.onicecandidate = ({candidate}) => signaling.send(JSON.stringify({toId: 2, data: candidate}));
  ...
  try {
     pc.createOffer().then((res)=>{
        pc.setLocalDescription(res).then(()=>{
            signaling.send( JSON.stringify({toId: 2, data:{desc: pc.localDescription}}));
        });
    })
...
start = () => {
    signaling.send(JSON.stringify({toId: 2, data: '发送给2'})) 
    // try {
